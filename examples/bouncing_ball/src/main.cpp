...
#include "graphics.h" // Include the ball image
...
uint16_t renderbuf[2][TFT_WIDTH];
...
void setup() {
  ...
  tft.drawBitmap(0, 0, (const uint8_t *)background, TFT_WIDTH, TFT_HEIGHT, GRIDCOLOR);
}
...
void loop() {
  ...
  if((ballx <= 15) || (ballx >= TFT_WIDTH - BALLWIDTH))
